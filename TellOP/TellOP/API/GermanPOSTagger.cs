// <copyright file="GermanPOSTagger.cs" company="University of Murcia">
...
namespace TellOP.Api
{
    ...
    /// </summary>
    public class GermanPOSTagger : OAuth2Api
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="GermanPOSTagger"/> class.
        /// </summary>
        /// <param name="account">The instance of the <see cref="Account"/> class to use to store the OAuth 2.0 account
        /// credentials.</param>
        public GermanPOSTagger(Account account)
            : base(Config.TellOPConfiguration.GetEndpointAsUri("TellOP.API.GermanPOSTagger"), HttpMethod.Get, account)
        {
        ...
        public GermanPOSTagger(Account account, string searchTerm)
            : base(new Uri(Config.TellOPConfiguration.GetEndpoint("TellOP.API.GermanPOSTagger") + "?q=" + Uri.EscapeDataString(Tools.StringUtils.Base64Encode(searchTerm))), HttpMethod.Get, account)
        {
        }
        ...
        public async Task<IList<GermanWord>> CallEndpointAsObjectAsync()
        {
            var r = await this.CallEndpointAsync().ConfigureAwait(false);
            var m = JsonConvert.DeserializeObject<List<GermanWord>>(r);
            return m;
        }
    }
}
