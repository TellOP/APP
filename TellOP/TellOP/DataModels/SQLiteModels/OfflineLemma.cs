...
namespace TellOP.DataModels.SQLiteModels
{
    ...
    public class OfflineLemma
    {
        ...
        [Indexed]
        [MaxLength(50)]
        public string Lemma { get; private set; } = null; // http://stackoverflow.com/a/40754/2875987
        ...
        [Indexed]
        [MaxLength(50)]
        private string Base { get; set; } = null; // http://stackoverflow.com/a/40754/2875987
        ...
        public static async Task<string> RetrieveBase(string lemma)
        {
            ...
            await Task.Run(() =>
            {
                lock (SQLiteManager.LocalLemmasDictionaryConnection)
                {
                    Expression<Func<OfflineLemma, bool>> exp = l => l.Lemma.Equals(lemma);
                    OfflineLemma query = SQLiteManager.LocalLemmasDictionaryConnection.Table<OfflineLemma>().Where(exp).FirstOrDefault();
                    if (query != null)
                    {
                        if (query.Base != null)
                        {
